                ...
                if dist < 4.5 then
                    if dist < Config.Locations[k].viewDistance then
                        ...
                        if IsControlJustReleased(0, 38) then
                                QBCore.Functions.Notify(Lang:t('notify.prepare'), 'success')
                                Wait(1000)
                                    ...
                                    QBCore.Functions.Notify(Lang:t('notify.rest') ..Config.RestingTime.. Lang:t('notify.seconds'), 'error')
                                    training = true
                                    resting = true
                                    CheckTraining()
                            elseif training == true then
                                QBCore.Functions.Notify(Lang:t('notify.needbreak'), 'primary')
                                resting = true
                                CheckTraining()
                        end
                    end
